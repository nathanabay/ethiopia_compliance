...
def run():
    print("--- 🛡️ APPLYING ROBUST PYTHON LOGIC (NO CRASHES) ---")
    
    # 1. TASS Sales Declaration
    sales_code = """
def execute(filters=None):
    # 1. SAFETY: Fill missing filters to prevent KeyError
    if not filters: filters = {}
    
    # Default Company to 'BESPO' if missing
    if not filters.get("company"): 
        filters["company"] = "BESPO"
        
    # Default Dates
    if not filters.get("from_date"): filters["from_date"] = "2026-01-01"
    ...
    # 2. QUERY: Use the filters safe in the knowledge they exist
    sql = \"\"\"
        SELECT 
...
def execute(filters=None):
    if not filters: filters = {}
    if not filters.get("company"): filters["company"] = "BESPO"
    if not filters.get("from_date"): filters["from_date"] = "2026-01-01"
    if not filters.get("to_date"): filters["to_date"] = frappe.utils.today()
